                                 ...
                                 [u32; 256])
{
    ...
    let mut binned_row_dark_level = 0_f32;
            ...
            if rownum & (binning - 1) == binning - 1 {
                ...
                if return_8 {
                    ...
                    for x in 0..binned_width {
                        ...
                        let binned_pixel_u8 = (binned_pixel >> (shift*2)) as u8;
                        histogram[binned_pixel_u8 as usize] += 1;
                        binned_image8_row[x] = binned_pixel_u8;
                    }
                } else {
                    assert!(return_binned_16);
                    for x in 0..binned_width {
                        let mut binned_pixel = binned_image16_row[x] as i16;
                        binned_pixel -= int_binned_row_dark_level;
                        if binned_pixel < 0 {
...
pub struct StarDescription {
    ...
    /// Number of pixels contributing to this centroid
    pub pixel_count: u32,
}
...
where i32: From<P>
{
    ...
    if binning != 1 {
        ...
        (brightness, num_saturated, peak_value, pixel_count) =
    ...
    Some(StarDescription{centroid_x: x + 0.5,
                         centroid_y: y + 0.5,
                         peak_value, brightness, num_saturated, pixel_count})
}
...
fn compute_brightness(image: &GrayImage, bounding_box: &Rect) -> (f64, u16, u8, u32) {
    ...
    let min_pixel_value: u8 = (peak_value as f32 * 0.5) as u8;
    ...
    (f64::max(sum, 0.0), num_saturated, peak_value, pixel_count)
}
                                ...
                                [u32; 256])
{
    ...
    // TODO: AI
    let min_size = 2; // Reject blobs with fewer pixels than this
...
//            }
//        }
        // This will ensure only blobs with a valid number of pixels (between min_size and max_size) are passed
        // to gate_star_2d() for further validation.
        for blob in form_blobs_from_candidates(candidates) {
            let blob_size = blob.candidates.len();
            if blob_size < min_size || blob_size > max_size as usize {
                continue; // Skip blobs that are too small or too large
            }

            if let Some(star) = gate_star_2d(&blob, image,
                                             image,
                                             binning, noise_estimate8, sigma,
                                             /*detect_hot_pixels=*/true,
                                             max_size, max_size) {
